...
public class ManageNeighbours implements Runnable {
    ...
    //Calculate download rate and retain 2 highest upploaders
    private void findNeighbours() throws IOException, InterruptedException {
        ...
        while (i < count) {
            ...
            boolean preferredPeerID = preferredPeerIDSet.contains(peerID);
            if (preferredPeerID == false) {
        ...
        for (Integer peerID : this.preferredPeerIDSet) {
            boolean setPreferredContains = SetPreferred.contains(peerID); 
            if (setPreferredContains == false) {
